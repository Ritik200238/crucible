...
export interface BuildOptions {
  /**
   * Whether execute and reconcile may act. False on a public HTTP instance
   * whose caller did not present the operator's token; the read tools still
   * answer in full.
   */
 ...
 *
 * A function rather than a module-level instance because the HTTP transport
 * is stateless: each request gets a fresh server and tears it down after. The
 * plan store above is shared on purpose — route and execute are two requests.
 */
export function buildServer(opts: BuildOptions): McpServer {
  const readOnly = (): ReturnType<typeof fail> =>
    fail(
      opts.readOnlyReason ??
        "This instance is read-only. execute and reconcile are not available without the operator's token.",
    );
  ...
  server.registerTool(
    ...
    async ({ planId }) => {
      if (!opts.mutationsAllowed) return readOnly();
      try {
...
// Run directly: serve over stdio for a local agent. Imported: only the builder,
// so the dashboard can mount the same tools on HTTP without a second stdin.
const runDirectly =
  process.argv[1] !== undefined && import.meta.url === pathToFileURL(resolve(process.argv[1])).href;
if (runDirectly) {
  const server = buildServer({ mutationsAllowed: true });
